...
from api import API
from mapping import (buy_type_map, decoration_upgrade_cost, element_map,
                     experience_level_up_map, inventory_type_map,
                     market_upgrade_cost_map, max_class_level_map,
                     scenario_id_map, summon_source_map, summon_type_map)
from qpyou import QPYOU
from tools import find, gen_clear_time, get_monster_name_by_id


class Bot:
    def __init__(
        self, user_="", user_mail_="", pw_="", device_id_="", device_=None, region_="eu"
    ):
        if not device_id_:
            try:
                ...
                with open("config.json", "w") as f:
                    ...
                    f.write(json.dumps(data))
        self.uid, self.did, self.sessionkey, appID = QPYOU(
            device_id_, device_
        ).hiveLogin(user_, pw_)
        self.bot = API(
            self.uid, self.did, user_, user_mail_, self.sessionkey, device_, appID
        )
        ...
        if not self.bot.daily_reward_info["is_checked"]:
            self.bot.log(
                "Collecting daily reward for day "
                "{}.".format(self.bot.daily_reward_info["check_count"])
            )
    ...
    def checkAndBuy(self, item_id, building_id, pos_x, pos_y):
        if (
            self.bot.wizard_info[
                buy_type_map[self.bot.shop_item_list[item_id]["buy_type"]]
            ]
            ...
            self.bot.log(
                "Not enough {type} ({amount}) to buy item. {cost} {type} needed.".format(
                    type=buy_type_map[self.bot.shop_item_list[item_id]["buy_type"]],
                    amount=self.bot.wizard_info[
                        buy_type_map[self.bot.shop_item_list[item_id]["buy_type"]]
                    ],
    ...
    def openShopSlots(self):
        ...
        while current_slots < 12:
            ...
            if self.bot.wizard_info[type_] >= upgrade_cost:
                ...
                self.bot.log(
                    "Upgraded Blackmarket slots to {}.".format(current_slots + 1)
                )
            else:
                self.bot.log(
                    "Not enough {} to open blackmarket slots to {}".format(
                        type_, current_slots + 1
                    )
                )
                return
            current_slots = self.bot.market_info["open_slots"]
    ...
    def swapUnitsInExpBuilding(self, unit_id_in, unit_id_out):
        ...
        unit_1_posx, unit_1_posy = (
            unit_list[unit_id_in]["pos_x"],
            unit_list[unit_id_in]["pos_y"],
        )
        unit_in_dict = {
            "unit_id": unit_1_id,
            "island_id": unit_1_island,
            "pos_x": unit_1_posx,
            "pos_y": unit_1_posy,
        }
        unit_2_id, unit_2_island = unit_id_out, unit_list[unit_id_out]["island_id"]
        unit_2_posx, unit_2_posy = (
            unit_list[unit_id_out]["pos_x"],
            unit_list[unit_id_out]["pos_y"],
        )
        unit_out_dict = {
            "unit_id": unit_2_id,
            "island_id": unit_2_island,
            "pos_x": unit_2_posx,
            "pos_y": unit_2_posy,
        }
    ...
    def checkCoordinates(self, island_id, pos_x, pos_y):
        if (
            (pos_x, pos_y) in self.bot.island_list[island_id]["base_coords"]
            and self.bot.island_list[island_id]["occupied_coords"][(pos_x, pos_y)][
                "occ"
            ]
            == 0
        ):
            ...
            distance = [
                abs(coord[0] - pos_x) + abs(coord[1] - pos_y)
                for coord in self.bot.island_list[island_id]["base_coords"]
                if self.bot.island_list[island_id]["occupied_coords"][
                    (coord[0], coord[1])
                ]
                == 0
            ]
            pos_x, pos_y = self.bot.island_list[island_id]["base_coords"][
                ...
                if self.getMonsterSpaceLeft() > 0:
                    try:
                        if self.bot.shop_item_list[1200005]["enable"] == 1:
                            if (
                                ...
                                if (
                                    self.bot.wizard_info["guild_point"]
                                    >= self.bot.shop_item_list[1200005]["buy_cost"][0]
                                ):
                                    pos_x = self.bot.buildings[2][
                                        "pos_x"
                                    ] + random.randint(-5, 5)
                                    pos_y = self.bot.buildings[2][
                                        "pos_y"
                                    ] + random.randint(-5, 5)
                                    pos_array = self.checkCoordinates(
                                        self.bot.buildings[2]["island_id"], pos_x, pos_y
                                    )
                                    pos_x, pos_y = pos_array[0], pos_array[1]
                                    if self.bot.BuyShopItem(
                                        1200005,
                                        self.bot.buildings[2]["island_id"],
                                        pos_x,
                                        pos_y,
                                    ):
                                    ...
                                    self.bot.log(
                                        "Not enough guild points ({}) to buy weekly rainbowmon. "
                                        "150 guild points needed.".format(
                                            self.bot.wizard_info["guild_point"]
                                        )
                                    )
                            else:
                                self.bot.log(
                                    ...
                                    "seconds remeining unitl next possible buy.".format(
                                        self.bot.shop_interval_list[1200005][
                                            "remained_time"
                                        ]
                                    )
                                )
                    ...
                    for unit, info_ in self.bot.unit_list.items():
                        if put_in_storage > 0:
                            ...
                            break
                    self.bot.log(
                        "Now attempting to move {} units to storage.".format(
                            len(unit_ids)
                        )
                    )
                ...
                if self.getMonsterSpaceLeft() > 0:
                    try:
                        if self.bot.shop_interval_list[901002]["remained_time"] <= 0:
                            ...
                            if (
                                self.bot.wizard_info["honor_point"]
                                >= self.bot.shop_item_list[901002]["buy_cost"][0]
                            ):
                                pos_x = self.bot.buildings[2]["pos_x"] + random.randint(
                                    -5, 5
                                )
                                pos_y = self.bot.buildings[2]["pos_y"] + random.randint(
                                    -5, 5
                                )
                                pos_array = self.checkCoordinates(
                                    self.bot.buildings[2]["island_id"], pos_x, pos_y
                                ...
                                self.bot.log(
                                    "Not enough honor points ({}) to buy weekly devilmon. "
                                    "180 honor points needed.".format(
                                        self.bot.wizard_info["honor_point"]
                                    )
                                )
                        else:
                            self.bot.log(
                                "Not able to buy weekly devilmon. {} seconds remeining until next "
                                "possible buy.".format(
                                    self.bot.shop_interval_list[901002]["remained_time"]
                                )
                            )
                            self.weekly_devilmon_bought = True
                    except KeyError:
    ...
    def buyPremiumPack(self):
        if (
            self.bot.wizard_info["wizard_crystal"]
            >= self.bot.shop_item_list[1100155]["buy_cost"][0]
        ):
            self.bot.BuyShopItem(1100155, 0, 0, 0)
        else:
            self.bot.log(
                "Not enough crystals ({}) to buy item. "
            ...
            for unit, info_ in self.bot.unit_list.items():
                if info_["building_id"] in building_ids:
                    if info_["unit_level"] < max_class_level_map[info_["class"]]:
                        if (
                            info_["exp_gained"]
                            >= experience_level_up_map[info_["class"]][
                                info_["unit_level"]
                            ]
                        ):
                            if self.bot.UpdateUnitExpGained([{"unit_id": unit}]):
                                self.bot.log(
                                    "Successfully leveled up unit {} in exp building.".format(
                                        unit
                                    )
                                )
                            else:
                                self.bot.log(
                                    "Error leveling up unit {} in exp building.".format(
                                        unit
                                    )
                                )
                    else:
                        self.bot.log(
                            "Unit {} already on max level {} for its class {}.".format(
                        ...
                        for unit_in, info_in in self.bot.unit_list.items():
                            if info_in["building_id"] == 0:
                                if (
                                    info_in["unit_level"]
                                    < max_class_level_map[info_in["class"]]
                                ):
                                    if info_in["unit_master_id"] not in [15105, 14314]:
                                        id_in = unit_in
                        if id_in:
                            self.bot.log(
                                "Swapping unit {} in exp building to unit {}".format(
                                    id_out, id_in
                                )
                            )
                            ...
                            self.bot.log(
                                "No suitabel unit found to swap into exp building."
                            )
        else:
    ...
    def refreshShop(self, crystal_refresh=False):
        ...
        if not crystal_refresh:
            ...
            self.last_shop_refresh = math.ceil(time.time()) - (
                3600 - self.bot.market_info["update_remained"]
            )
            for i, item in enumerate(self.bot.market_list):
                if item["item_master_type"] == 1:
                    ...
                    self.bot.log(
                        "{}: Monster - {} {} {}".format(
                            i + 1, monster, element, item["item_master_id"]
                        )
                    )
                elif item["item_master_type"] == 9:
                    self.bot.log(
                        "{}: Scroll - {}, amount: {}".format(
                            i + 1,
                            summon_source_map[item["item_master_id"]],
                            item["amount"],
                        )
                    )
                elif item["item_master_type"] == 8:
                    self.bot.log(
                        "{}: Rune - {}".format(i + 1, Bot.parseRune(item["runes"][0]))
                    )
                else:
                    self.bot.log(
                        "{}: Unknown shop item type {}".format(
                            i + 1, item["item_master_type"]
                        )
                    )
            ...
            for item in self.bot.market_list:
                if item["item_master_type"] == 1:
                    ...
                    self.bot.log(
                        "{}: Scroll {}, amount: {}".format(
                            i, summon_source_map[item["item_master_id"]], item["amount"]
                        )
                    )
                    i += 1
                elif item["item_master_type"] == 8:
                    self.bot.log(
                        "{}: Rune {}".format(i, Bot.parseRune(item["runes"][0]))
                    )
                    i += 1
                else:
                    self.bot.log(
                        "{}: Unknown shop item type {}".format(
                            i, item["item_master_type"]
                        )
                    )
                    i += 1
                ...
                if item["item_master_type"] in item_list_:
                    if item["item_master_type"] == 8:
                        if self.checkRuneFilter(item["runes"][0]):
                            if (
                                self.bot.wizard_info["wizard_mana"] >= item["buy_mana"]
                                and item["available"]
                            ):
                                self.bot.log(
                                    "Bought rune {} for {} mana.".format(
                                        Bot.parseRune(item["runes"][0]),
                                        item["buy_mana"],
                                    )
                                )
                                self.bot.BuyBlackMarketItem(
                                    item["item_no"],
                                    item["item_master_type"],
                                    item["item_master_id"],
                                    item["amount"],
                                )
                            else:
                        ...
                        if (
                            self.bot.wizard_info["wizard_mana"] >= item["buy_mana"]
                            and item["available"]
                            and item["item_master_id"] in self.scroll_types
                        ):
                            self.bot.log(
                                "Bought item {} for {} mana.".format(
                                    summon_source_map[item["item_master_id"]],
                                    item["buy_mana"],
                                )
                            )
                            ...
                            self.bot.log(
                                "Not the right scroll item to buy: "
            ...
            if rune["Quality"] > 2:
                ...
                if rune["Stars"] >= 4:
                    if rune["Slot"] == 1:
                        ...
                        if (
                            rune["Main Stat"] < 6 and rune["Main Stat"] % 2 == 1
                        ) or rune["Main Stat"] == 9:
                            return False
                        else:
                            return True
    ...
    def parseRune(rune_data):
        if rune_data:
            ...
            if innate_stat_old:
                ...
                innatestat_dic.update(
                    {"Innate Stat": innate_stat, "Innate Stat value": innate_stat_value}
                )
            ...
            if len(substats) >= 1:
                ...
                substat_dic.update(
                    {"Substat 1": substat_1, "Substat 1 value": substat_1_value}
                )
            ...
            if len(substats) >= 2:
                ...
                substat_dic.update(
                    {"Substat 2": substat_2, "Substat 2 value": substat_2_value}
                )
            ...
            if len(substats) >= 3:
                ...
                substat_dic.update(
                    {"Substat 3": substat_3, "Substat 3 value": substat_3_value}
                )
            ...
            if len(substats) >= 4:
                ...
                substat_dic.update(
                    {"Substat 4": substat_4, "Substat 4 value": substat_4_value}
                )
            ...
            rune = {
                ...
                "Quality": quality,
                "Main Stat": main_stat,
                "Main Stat value": main_stat_value,
            }
    ...
    def upgradeRuneTo(self, rune_id, upgrade_curr, upgrade=15):
        ...
        while upgrade_curr < upgrade:
            ...
            if new_rune_data["ret_code"] == 0:
                if old >= upgrade_curr:
                    self.bot.log(
                        "Upgrade failed, rune {} still at level {}".format(
                            rune_id, upgrade_curr
                        )
                    )
                else:
                    self.bot.log(
                        "Upgrade successful, rune {} now at level {}".format(
                            rune_id, upgrade_curr
                        )
                    )
                ...
                self.bot.log(
                    "Error while upgrading rune {}. "
                    "Maybe not enough mana. Rune at level {}.".format(
                        rune_id, upgrade_curr
                    )
                )
                break
    ...
    def doDailyWish(self):
        if 10 in self.bot.buildings:
            ...
            self.bot.log(
                "Wish granted {} with amount of {}".format(
                    inventory_type_map[wish_outcome["item_master_type"]],
                    wish_outcome["amount"],
                )
            )
        else:
    ...
    def collectAllSocialPoints(self):
        collecible_points = (
            self.bot.wizard_info["social_point_max"]
        ...
        if collecible_points > 0:
            ...
            if collectible_points_list:
                ...
                self.bot.log(
                    "Collected all social points. Current social points {}".format(
                        self.bot.wizard_info["social_point_current"]
                    )
                )
                return
                ...
                self.bot.log(
                    "No social points found in mail list. Returning to other activities."
                )
                return
        else:
            self.bot.log(
                "Not able to collect any more social points. Current social points: {}".format(
                    self.bot.wizard_info["social_point_current"]
                )
            )
    ...
    def doDailyQuests(self):
        ...
        if not self.bot.daily_quest_list[9]["completed"] == 1:
            ...
            self.bot.doDungeon(
                8001, 1, clear_time, units=None, helper_list=helper, win_lose=1
            )
            helper_used += 1
            if self.bot.daily_quest_list[9]["completed"] == 1:
                self.bot.log(
                    "Successfully finished giants dungeon quest. Now collecting reward."
                )
                self.collectDailyRewards()
        elif not self.bot.daily_quest_list[9]["rewarded"] == 1:
            self.bot.log(
                "Daily giants dungeon quest already finished, but not collected yet. Collecting reward now."
            )
            self.collectDailyRewards()
        else:
        ...
        if 1001 in [
            dungeon["dungeon_id"] for dungeon in self.bot.dungeon_list
            ...
            if not self.bot.daily_quest_list[10]["completed"] == 1:
                ...
                helper = self.getNextBestRep()
                self.bot.doDungeon(
                    1001, 1, clear_time, units=None, helper_list=helper, win_lose=1
                )
                helper_used += 1
                if self.bot.daily_quest_list[10]["completed"] == 1:
                    self.bot.log(
                        "Successfully finished monday elemental dungeon quest. Now collecting reward."
                    )
                    self.collectDailyRewards()
            elif not self.bot.daily_quest_list[10]["rewarded"] == 1:
                self.bot.log(
                    "Monday elemental dungeon quest already finished, but not collected yet. "
                    "Collecting reward now."
                )
                self.collectDailyRewards()
            else:
                self.bot.log(
                    "Monday elemental dungeon quest already finished and collected."
                )
        ...
        if 2001 in [
            ...
            if not self.bot.daily_quest_list[11]["completed"] == 1:
                ...
                helper = self.getNextBestRep()
                self.bot.doDungeon(
                    2001, 1, clear_time, units=None, helper_list=helper, win_lose=1
                )
                helper_used += 1
                if self.bot.daily_quest_list[11]["completed"] == 1:
                    self.bot.log(
                        "Successfully finished tuesday elemental dungeon quest. Now collecting reward."
                    )
                    self.collectDailyRewards()
            elif not self.bot.daily_quest_list[11]["rewarded"] == 1:
                self.bot.log(
                    "Tuesday elemental dungeon quest already finished, but not collected yet. "
                    "Collecting reward now."
                )
                self.collectDailyRewards()
            else:
                self.bot.log(
                    "Tuesday elemental dungeon quest already finished and collected."
                )
        ...
        if 3001 in [
            ...
            if not self.bot.daily_quest_list[12]["completed"] == 1:
                ...
                helper = self.getNextBestRep()
                self.bot.doDungeon(
                    3001, 1, clear_time, units=None, helper_list=helper, win_lose=1
                )
                helper_used += 1
                if self.bot.daily_quest_list[12]["completed"] == 1:
                    self.bot.log(
                        "Successfully finished wednesday elemental dungeon quest. Now collecting reward."
                    )
                    self.collectDailyRewards()
            elif not self.bot.daily_quest_list[12]["rewarded"] == 1:
                self.bot.log(
                    "Wednesday elemental dungeon quest already finished, but not collected yet. "
                    "Collecting reward now."
                )
                self.collectDailyRewards()
            else:
                self.bot.log(
                    "Wednesday elemental dungeon quest already finished and collected."
                )
        ...
        if 4001 in [
            ...
            if not self.bot.daily_quest_list[13]["completed"] == 1:
                ...
                helper = self.getNextBestRep()
                self.bot.doDungeon(
                    4001, 1, clear_time, units=None, helper_list=helper, win_lose=1
                )
                helper_used += 1
                if self.bot.daily_quest_list[13]["completed"] == 1:
                    self.bot.log(
                        "Successfully finished thursday elemental dungeon quest. Now collecting reward."
                    )
                    self.collectDailyRewards()
            elif not self.bot.daily_quest_list[13]["rewarded"] == 1:
                self.bot.log(
                    "Thursday elemental dungeon quest already finished, but not collected yet. "
                    "Collecting reward now."
                )
                self.collectDailyRewards()
            else:
                self.bot.log(
                    "Thursday elemental dungeon quest already finished and collected."
                )
        ...
        if 7001 in [
            ...
            if not self.bot.daily_quest_list[15]["completed"] == 1:
                ...
                helper = self.getNextBestRep()
                self.bot.doDungeon(
                    7001, 1, clear_time, units=None, helper_list=helper, win_lose=1
                )
                helper_used += 1
                if self.bot.daily_quest_list[15]["completed"] == 1:
                    self.bot.log(
                        "Successfully finished sunday elemental dungeon quest. Now collecting reward."
                    )
                    self.collectDailyRewards()
            elif not self.bot.daily_quest_list[15]["rewarded"] == 1:
                self.bot.log(
                    "Sunday elemental dungeon quest already finished, but not collected yet. "
                    "Collecting reward now."
                )
                self.collectDailyRewards()
            else:
                self.bot.log(
                    "Sunday elemental dungeon quest already finished and collected."
                )
        ...
        if 5001 in [
            ...
            if not self.bot.daily_quest_list[14]["completed"] == 1:
                while helper_used < 3:
                    ...
                    helper = self.getNextBestRep()
                    self.bot.doDungeon(
                        5001, 1, clear_time, units=None, helper_list=helper, win_lose=1
                    )
                    helper_used += 1
                if self.bot.daily_quest_list[14]["completed"] == 1:
                    self.bot.log(
                        "Successfully finished magic dungeon quest. Now collecting reward."
                    )
                    self.collectDailyRewards()
            elif not self.bot.daily_quest_list[14]["rewarded"] == 1:
                self.bot.log(
                    "Magic dungeon quest already finished, but not collected yet. Collecting reward now."
                )
                self.collectDailyRewards()
            else:
        ...
        if not self.bot.daily_quest_list[1]["completed"] == 1:
            while not self.bot.daily_quest_list[1]["completed"] == 1:
                ...
                self.bot.do_scenario(
                    3, 7, 1, clear_time, units=None, helper_list=None, win_lose=1
                )
            if self.bot.daily_quest_list[1]["completed"] == 1:
                self.bot.log(
                    "Successfully finished energy quest. Now collecting reward."
                )
                self.collectDailyRewards()
        elif not self.bot.daily_quest_list[1]["rewarded"] == 1:
            self.bot.log(
                "Daily energy quest already finished, but not collected yet. Collecting reward now."
            )
            self.collectDailyRewards()
        else:
        ...
        if not self.bot.daily_quest_list[4]["completed"] == 1:
            while not self.bot.daily_quest_list[4]["completed"] == 1:
                ...
                for rune, info_ in self.bot.rune_list.items():
                    if info_["upgrade_curr"] < 6:
                        ...
                        self.bot.log(
                            "Found rune to upgrade {} with current upgrade {}".format(
                                rune_to_upgrade, upgrade_curr
                            )
                        )
                        break
                if rune_to_upgrade:
            ...
            self.bot.log(
                "Daily rune quest already finished, but not collected yet. Collecting reward now."
            )
            self.collectDailyRewards()
        else:
        ...
        if not self.bot.daily_quest_list[2]["completed"] == 1:
            while not self.bot.daily_quest_list[2]["completed"] == 1:
                ...
                unit_to_use = None
                for unit, info_ in self.bot.unit_list.items():
                    if info_["building_id"] == 0 and info_["unit_master_id"] not in [
                        15105,
                        14314,
                    ]:
                        if (
                            info_["class"] >= 3
                            and info_["unit_level"]
                            < max_class_level_map[info_["class"]]
                        ):
                ...
                if unit_to_upgrade and unit_to_use:
                    self.bot.SacrificeUnit(
                        target_id=unit_to_upgrade,
                        source_list=[{"source_id": unit_to_use}],
                        island_id=island_id,
            ...
            if self.bot.daily_quest_list[2]["completed"] == 1:
                self.bot.log(
                    "Successfully finished upgrading quest. Now collecting reward."
                )
                self.collectDailyRewards()
        elif not self.bot.daily_quest_list[2]["rewarded"] == 1:
            self.bot.log(
                "Daily upgrading quest already finished, but not collected yet. Collecting reward now."
            )
            self.collectDailyRewards()
        else:
        ...
        if not self.bot.daily_quest_list[7]["completed"] == 1:
            ...
            if self.bot.daily_quest_list[7]["completed"] == 1:
                self.bot.log(
                    "Successfully finished helper quest. Now collecting reward."
                )
                self.collectDailyRewards()
        elif not self.bot.daily_quest_list[7]["rewarded"] == 1:
            self.bot.log(
                "Daily helper quest already finished, but not collected yet. Collecting reward now."
            )
            self.collectDailyRewards()
        else:
        ...
        if not self.bot.daily_quest_list[3]["completed"] == 1:
            ...
            if summon_progress > 0:
                self.bot.log(
                    "Continuing daily summoning quest. Already {} summons done.".format(
                        summon_progress
                    )
                )
            else:
                ...
                if self.getMonsterSpaceLeft() > 0:
                    ...
                    if summon_result == 0:
                        ...
                        if summon_mode == 5:
                            ...
                            self.bot.log(
                                "Not enough social points or Unknown Scrolls. Stopping the summoning quest."
                            )
                            break
                else:
                    self.bot.log(
                        "Couldn't finish quest of summoning 3 Monsters. Did only {} summons, "
                        "now trying to put monsters in storage.".format(summon_progress)
                    )
                    put_in_storage = (
                        self.getMonsterSpaceLeft() + summon_end - summon_progress
                    )
                    unit_ids = []
                    for unit, info_ in self.bot.unit_list.items():
                        if put_in_storage > 0:
                    ...
                    if not self.moveUnitsToStorage(unit_ids):
                        self.bot.log(
                            "Not able to make space for all daily summons. "
                            "Stopping for now after {} summons.".format(summon_progress)
                        )
                        break
            if (
                summon_progress == summon_end
                and self.bot.daily_quest_list[3]["completed"] == 1
            ):
                self.bot.log(
                    "Successfully finished summoning quest. Now collecting reward."
                )
                self.collectDailyRewards()
        elif not self.bot.daily_quest_list[3]["rewarded"] == 1:
            self.bot.log(
                "Daily summoning quest already finished, but not collected yet. Collecting reward now."
            )
            self.collectDailyRewards()
        else:
        ...
        # send social points
        if (
            not self.bot.daily_quest_list[6]["completed"] == 1
            and len(self.bot.friend_list) >= 5
        ):
            sending_progress = self.bot.daily_quest_list[6]["progressed"]
            if sending_progress > 0:
                self.bot.log(
                    "Continuing sending social points quest. Already {} times sent.".format(
                        sending_progress
                    )
                )
            else:
            ...
            if sent >= 5:
                if self.bot.daily_quest_list[6]["completed"] == 1:
                    self.bot.log(
                        "Successfully finished sending social points quest. Now collecting reward."
                    )
                    self.collectDailyRewards()
            else:
                self.bot.log(
                    "Can't send enough social points. "
                ...
                while sent < 5:
                    ...
                    if summon_response == 0:
                        ...
                        sent += next_send
                        self.bot.log(
                            "Sent social points to {} more friends.".format(next_send)
                        )
                        ...
                        if not self.moveUnitsToStorage(unit_ids):
                            self.bot.log(
                                "Not able to make space for social summon. "
            ...
            self.bot.log(
                "Not able to finish sending social points quest. "
            ...
            self.bot.log(
                "Sending social points quest already finished, but not collected yet. Collecting reward now."
            )
            self.collectDailyRewards()
        else:
        ...
        if not self.bot.daily_quest_list[8]["completed"] == 1:
            ...
            if (
                self.bot.daily_quest_list[8]["completed"] == 1
                and not self.bot.daily_quest_list[8]["rewarded"] == 1
            ):
                self.bot.log(
                    "Successfully finished arena quest. Now collecting reward."
                )
                self.collectDailyRewards()
        elif not self.bot.daily_quest_list[8]["rewarded"] == 1:
            self.bot.log(
                "Daily arena quest already finished, but not collected yet. Collecting reward now."
            )
            self.collectDailyRewards()
        else:
    ...
    def moveUnitsToStorage(self, unit_ids):
        space_in_storage_left = (
            self.bot.wizard_info["unit_slots"]["number"]
            + self.bot.unit_depository_slots["number"]
            - len(self.bot.unit_list)
        ...
        if space_in_storage_left > 0:
            ...
            for unit_id in unit_ids:
                if space_in_storage_left > 0:
                    pos_x = min(
                        max(self.bot.buildings[25]["pos_x"] + random.randint(-5, 5), 0),
                        21,
                    )
                    pos_y = min(
                        max(self.bot.buildings[25]["pos_y"] + random.randint(-5, 5), 0),
                        21,
                    )
                    move_list.append(
                        OrderedDict(
                    ...
                    self.bot.log(
                        "Running out of space to move all {} into storage. "
            ...
            self.bot.log(
                "Not enough space in monster storage to store monster(s) {}.".format(
                    unit_ids
                )
            )
            return False
        ...
        try:
            if cash_used:
                ...
                if (
                    ...
                    > self.bot.unit_depository_slots["upgrade"]["mana"]
                ):
    ...
    def finishScenario(self, difficulty):
        ...
        for scenario in scenarios:
            ...
            if scenario["cleared"] == 1 and scenario["difficulty"] == difficulty:
                ...
                for ach in achievements:
                    quest_id = 0
                    if (
                        int(ach["required level"])
                        > self.bot.wizard_info["wizard_level"]
                    ):
                        break
                    ...
                    if quest_id > 0:
                        if quest_id not in self.bot.quest_rewarded:
                            if not self.bot.quest_active[quest_id]["is_completed"]:
                                for condition_ in self.bot.quest_active[quest_id][
                                    "conditions"
                                ]:
                                    self.bot.UpdateAchievement(
                                        [
                                            {
                                                "ach_id": quest_id,
                                                "cond_id": condition_[0],
                                                "current": condition_[1],
                                            }
                                        ]
                                    )
        ...
        if not scenarios_to_go:
            max_scenario = [
                scenario["region_id"]
                for scenario in map(dict, self.bot.scenario_list)
                if scenario["difficulty"] == difficulty
            ]
            print(max_scenario)
            scenarios_not_started = (
                range(max(max_scenario) + 1, 14) if max(max_scenario) != 13 else []
            )
        else:
            scenarios_not_started = (
                range(max(scenarios_to_go) + 1, 14)
                if max(scenarios_to_go) != 13
                else []
            )
        print(scenarios_to_go)
        print(scenarios_not_started)
        for scenario, info_ in scenarios_to_go.items():
            stage_no_cleared = max(
                [
                    stage["stage_no"]
                    for stage in info_["stage_list"]
                    if stage["cleared"] == 1
                ]
            )
            while stage_no_cleared < 7:
                ...
                if not self.bot.do_scenario(
                    scenario, stage_no_cleared, difficulty, clear_time
                ):
                    return False
        ...
        if space_left > 0:
            ...
            for unit_id in unit_ids:
                if space_left > 0:
                    ...
                    self.bot.log(
                        "Running out of space to get all {} out of storage. "
            ...
            self.bot.log(
                "Not enough space in inventory to take monster(s) {} out of storage.".format(
                    unit_ids
                )
            )
        return False
        ...
        if self.bot.pvp_info.get("rating_remained") <= 601200:
            ...
            for wizard, info_ in self.bot.arena_log.items():
                if info_["STATUS"] == 0:
                    ...
                    if (
                        info_["opp_arena_score"] <= self.getArenaRating() + 100
                        or len(opp_units) <= 2
                        or max(
                            [
                                opp_unit["unit_info"]["unit_level"]
            ...
            for wizard, info_ in self.bot.arena_list.items():
                opp_units = self.bot.GetArenaUnitList(wizard)["opp_unit_list"]
                if (
                    info_["arena_score"] <= self.getArenaRating() + 100
                    or len(opp_units) <= 2
                    or max(
                        [opp_unit["unit_info"]["unit_level"] for opp_unit in opp_units]
                    )
                    <= 30
                ):
            ...
            if revenge[0] in [enemy for enemy in self.bot.arena_log]:
                if self.bot.wizard_info["arena_energy"] > 0:
                    if self.getArenaRating() > 1150:
                        self.bot.log(
                            "Battling revenge Summoner in Arena: {}".format(revenge[0])
                        )
                        self.bot.log(
                            "Attempt to lose because of high arena rating: {}".format(
                                self.getArenaRating()
                            )
                        )
                        self.bot.doArena(
                            revenge[0], units, win_lose=0, log_id=revenge[1]
                        )
                    else:
                        self.bot.log(
                            "Battling revenge Summoner in Arena: {}".format(revenge[0])
                        ...
                        self.bot.doArena(
                            revenge[0], units, win_lose=1, log_id=revenge[1]
                        )
                    self.bot.GetArenaLog()
                else:
                    self.bot.log("Not enough arena wings to do more battles.")
    ...
    def sendSocialPoints(self):
        ...
        for friend, info_ in self.bot.friend_list.items():
            if (
                ...
                self.bot.log(
                    "Maximum social points reached. Stop sending social points for now."
                )
                break
            ...
            for friend, info_ in self.bot.helper_list.items():
                if int(info_["next_assist_time"]) <= 0:
                    reps.append(
                        {
                            "wizard_id": friend,
                            "unit_id": info_["rep_unit_id"],
                            "arena_score": info_["arena_score"],
                            "wizard_level": info_["wizard_level"],
                        }
                    )
            return reps
        else:
    ...
    def getNextBestRep(self):
        ...
        if rep_list:
            try:
                max_arena_score_index = find(
                    rep_list,
                    "arena_score",
                    max(
                        [
                            friend["arena_score"]
                            for friend in rep_list
                            if friend["wizard_level"] > 15
                        ]
                    ),
                )
            except ValueError:
            ...
            ret = [
                {
                    "wizard_id": rep_list[max_arena_score_index]["wizard_id"],
    ...
    def doSocialSummon(self):
        if self.getMonsterSpaceLeft() > 0:
            if self.bot.wizard_info["social_point_current"] >= 100:
                ...
                self.bot.log(
                    "Not enough social points to complete {}: "
                    "{}".format(
                        summon_type_map[5], self.bot.wizard_info["social_point_current"]
                    )
                )
                return 1
        else:
            self.bot.log(
                "Not enough space to summon. Maybe put monsters in storage. "
            ...
            if self.bot.wizard_info["wizard_mana"] >= 300:
                ...
                for inventory_item, info_ in self.bot.inventory_list.items():
                    if info_["item_master_type"] == 9 and info_["item_master_id"] == 1:
                        inventory_scroll_index = self.bot.inventory_list.index(
                            inventory_item
                        )
                        break
                if (
                    inventory_scroll_index >= 0
                    and self.bot.inventory_list[inventory_scroll_index]["item_quantity"]
                    > 0
                ):
                    ...
                    self.bot.log(
                        "None or not enough items found to do summon {}.".format(
                            summon_type_map[1]
                        )
                    )
                    return 1
                ...
                self.bot.log(
                    "Not enough mana to complete {}: {}".format(
                        summon_type_map[1], self.bot.wizard_info["wizard_mana"]
                    )
                )
                return 1
        else:
    ...
    def check_finished_ach(self):
        for ach in self.bot.quest_rewarded:
            if ach in self.bot.quest_active:
                self.bot.log(
                    "Error: completed quest {} also in active quests".format(ach)
                )
    ...
    def check_open_ach(self):
        ...
        for open_ach, item_ in self.bot.quest_active.items():
            ...
            try:
                ...
                open_quest_list.append(
                    [
                        open_ach,
                        item_["conditions"],
                        achievements[index].get("title_en"),
        ...
        try:
            ...
            if level < 10:
                ...
                if self.bot.wizard_info["honor_point"] >= cost:
                    ...
                    self.bot.log(
                        "Not enough honor points ({}) to upgrade "
                        "sanctum of energy to level {}".format(
        ...
        try:
            ...
            if level < 10:
                ...
                if self.bot.wizard_info["honor_point"] >= cost:
                    ...
                    self.bot.log(
                        "Not enough honor points ({}) to upgrade myterious plant to level {}".format(
                            self.bot.wizard_info["honor_point"], level + 1
                        )
                    )
            else:
        ...
        if len(self.bot.friend_list) < 50:
            ...
            if missing_friends > 0:
                if friend_requests["friend_req_list"]:
                    for new_friend in friend_requests["friend_req_list"]:
                        if self.bot.AcceptFriendRequest(
                            new_friend["wizard_id"], new_friend["wizard_name"]
                        ):
                            missing_friends -= 1
                        else:
                            self.bot.log(
                                "Not able to accept request from {}".format(
                                    new_friend["wizard_name"]
                                    if new_friend["wizard_name"]
                                    else new_friend["wizard_id"]
                                )
                            )
                        if missing_friends == 0:
                            break
            if missing_friends > 0:
                ...
                for recommended_friend in recommended_friends["recommended_list"]:
                    if recommended_friend["wizard_id"] not in [
                        friend for friend, info_ in self.bot.friend_list.items()
                    ] and recommended_friend["wizard_id"] not in [
                        pending["wizard_id"] for pending in pending_requests
                    ]:
                        if self.bot.AddFriendRequestByUid(
                            recommended_friend["channel_uid"],
                            recommended_friend["wizard_name"],
                        ):
                            missing_friends -= 1
                        else:
                            self.bot.log(
                                "Not able to send friend request to {}".format(
                                    recommended_friend["wizard_name"]
                                    if recommended_friend["wizard_name"]
                                    else recommended_friend["wizard_id"]
                                )
                            )
                    else:
                        self.bot.log(
                            "Already sent friend request to {} or already in friend list.".format(
                                recommended_friend["wizard_name"]
                                if recommended_friend["wizard_name"]
                                else recommended_friend["wizard_id"]
                            )
    ...
    while True:
        try:
            ...
            for building, info in autoplay.bot.buildings.items():
                if "harvest_available" in info and info["harvest_available"] > 0:
                    autoplay.bot.Harvest(info["building_id"])
                    time.sleep(random.uniform(0.5, 2.5))
            if 0 in [
                info["completed"]
                for quest, info in autoplay.bot.daily_quest_list.items()
            ] or 0 in [
                info["rewarded"]
                for quest, info in autoplay.bot.daily_quest_list.items()
            ]:
            ...
            if autoplay.weekly_devilmon_bought:
                ...
                autoplay.bot.log(
                    "Prioritizing weekly devilmon before energy buildings."
                )
            ...
            sleeper = max(
                (
                    autoplay.bot.market_info["update_remained"]
                    - (time.time() - start)
                    + random.randint(1, 15)
                )
                / 2,
