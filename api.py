...
class API(object):
    ...
    GAME_INDEX = 2624
    PROTO_VER = 11200
    BINARY_SIZE = 29467845
    BINARY_CHECK = "0464cf0ba854b0e7f1e762f3154f0258"
    ...
    def __init__(
        ...
        if app_id and "android" in str(app_id):
            ...
            headers = {
                "User-Agent": API.USER_AGENT_SCRAPE,
            ...
            sess_ver.close()
            if len(version) > len(self.app_version) or any(
                [
                    int(net_version[i]) > int(given_version[i])
            ...
            version_req = requests.get(
                "https://itunes.apple.com/de/app/summoners-war-sky-arena/id852912420"
            ).text
            soup = BeautifulSoup(version_req, "html.parser")
            version = None
            while not version:
                try:
                    version = (
                        soup.find(
                            "p",
                            {
                                "class": "l-column small-6 medium-12 whats-new__latest__version"
                            },
                        )
        ...
        self.c2_status = None
        self.c2_api = None
        self.last_position = {"island_id": 1, "pos_x": 10, "pos_y": 12}
        if id_ and email:
    ...
    def set_region(self, region=None):
        ...
        if region not in regions:
            self.log(
                "invalid region, choose one from these: {}".format(",".join(regions))
            )
            # exit(1)
        ...
        try:
            ...
            try:
                res = self.s.post(
                    ...
                    data,
                    headers={
                        "SmonTmVal": str(old_data["ts_val"])
                        if old_data
                        else str(
                            self.crypter.get_player_server_connect_elapsed_time(ts)
                        ),
                        "SmonChecker": self.crypter.get_smon_checker(data, ts),
                    },
                )
            except KeyError:
    ...
    def getLocation(self):
        ...
        locations = json.loads(
            self.crypter.decrypt_response(
                self.s.get(self.c2_location).content,
                2 if "_c2.php" in self.c2_location else 1,
            )
        )
        self.c2_status = locations["server_url_list"][regions.index(self.region)][
            "status"
        ]
        self.c2_version = locations["server_url_list"][regions.index(self.region)][
            "version"
        ]
        self.c2_api = locations["server_url_list"][regions.index(self.region)][
            "gateway"
        ]
        if self.debug == 1:
            self.log(
                "{} {} {} {}".format(
                    self.c2_location, self.c2_status, self.c2_version, self.c2_api
                )
            )
        ...
        if kind == 1:
            data = OrderedDict(
                [
                    ("command", cmd),
                    ("game_index", self.game_index),
                    ("session_key", self.getUID()),
                    ("proto_ver", self.proto_ver),
                    ("infocsv", self.infocsv),
                    ("channel_uid", self.uid),
                ]
            )
        elif kind == 2:
    ...
    def AcceptFriendRequest(self, req_wizard_id, req_wizard_name=""):
        self.log(
            "Accepting friend request from: {}".format(
    ...
    def AddFriendRequestByUid(self, uid, add_wizard_name=""):
        self.log(
            "Sending friend request to: {}".format(
    ...
    def BuyInappProduct(
        self, product_id, product_price, currency, signature, receipt_data
    ):
    ...
    def UpdateDailyQuest(self, quests):
        ...
        if (
            res
            and sum(
                [info["completed"] for quest, info in self.daily_quest_list.items()]
            )
            == len(self.daily_quest_list) - 1
        ):
    ...
    def BattleArenaResult(
        self, battle_key, opp_unit_status_list, unit_id_list, win_lose
    ):
    ...
    def BattleScenarioStart(
        self, region_id, stage_no, difficulty, unit_id_list, helper_list=None
    ):
    ...
    def SummonUnit(self, mode):
        ...
        extra_data = OrderedDict(
            [
                ("building_id", self.buildings[2]["building_id"]),
                ("mode", mode),
                (
                    "pos_arr",
                    [
                        OrderedDict(
                            [
                                ("island_id", self.buildings[2]["island_id"]),
    ...
    def SacrificeUnit(
        self, target_id, source_list, island_id, pos_x, pos_y, building_id=0
    ):
    ...
    def DoRandomWishItem(self, crystal_refresh=False):
        ...
        extra_data = OrderedDict(
            [
                ("island_id", self.buildings[10]["island_id"]),
    ...
    def GetRiftOfWorldsRaidServerInfo(
        self, join_type, raid_id, stage_id, speed=2, secret=1
    ):
    ...
    def BattleRiftOfWorldsRaidResult(
        self,
        battle_key,
        win_lose,
        opp_unit_status_list,
    ...
    def _getUserInfo(self):
        if hasattr(self, "wizard_info"):
            return (
                "username: {}, level: {}, energy: {}, mana: {}, crystal: {}, honor point: {}, guild_point: {}, "
                "social_points: {}, arena energy: {}".format(
                    self.wizard_info["wizard_name"],
    ...
    def login(self):
        ...
        while status == 1:
            ...
            self.log(
                "Server {} undergoing maintenance right now, waiting for {} seconds.".format(
    ...
    def CheckCandidateUid(self, guest_uid, hive_uid, wizard_id):
        data = OrderedDict(
            [
                ("command", "CheckCandidateUid"),
                ("proto_ver", self.proto_ver),
                ("infocsv", self.infocsv),
                ("uid", guest_uid),
                ("channel_uid", guest_uid),
                ("candidate_uid", hive_uid),
                ("wizard_id", wizard_id),
            ]
        )
        res = self.call_api(self.c2_api, data)
    ...
    def ProcessGuestTransition(self, guest_uid, hive_uid, wizard_id, use_old=0):
        data = OrderedDict(
            [
                ("command", "ProcessGuestTransition"),
                ("proto_ver", self.proto_ver),
                ("infocsv", self.infocsv),
                ("uid", guest_uid),
                ("channel_uid", hive_uid),
                ("did", self.did),
                ("game_index", self.game_index),
                ("wizard_id", wizard_id),
                ("candidate_uid", hive_uid),
    ...
    def parseBattleStart(input_, win_lose=1):
        ...
        if win_lose:
            if "opp_unit_list" in list(input_.keys()) and "pvp_info" not in list(
                input_.keys()
            ):
                opp_unit_status_list = [
                    {"unit_id": item["unit_id"], "result": 2}
                    for item in itertools.chain.from_iterable(input_["opp_unit_list"])
                ]
                ...
                opp_unit_status_list = [
                    ...
                    for item in itertools.chain.from_iterable(
                        input_["dungeon_unit_list"]
                    )
                ]
                for alive_unit in opp_unit_status_list[-2:]:
                    alive_unit["result"] = 1
                return battle_key, opp_unit_status_list
            elif "opp_unit_list" in list(input_.keys()) and "pvp_info" in list(
                input_.keys()
            ):
                opp_unit_status_list = [
                    {"unit_id": item["unit_info"]["unit_id"], "result": 2}
                    for item in input_["opp_unit_list"]
                ]
                return battle_key, opp_unit_status_list
                ...
                opp_unit_status_list = [
                    {
                        ...
                        "result": 1 if item["boss"] == 1 else 2,
                    }
                    for item in itertools.chain.from_iterable(
                        input_["trial_tower_unit_list"]
                    )
    ...
    def doDungeon(
        self, dungeon_id, stage_id, clear_time, units=None, helper_list=None, win_lose=1
    ):
        if dungeon_id not in [dungeon["dungeon_id"] for dungeon in self.dungeon_list]:
            ...
            return
        unit_id_list = (
            [{"unit_id": unit["unit_id"]} for unit in self.defense_unit_list]
            if not units
            else units
        )
        ...
        if not dungeon_start:
            self.log(
                "Dungeon: {}, stage: {} not successfully started".format(
                    dungeon_id, stage_id
                )
            )
            return
        ...
        dungeon_end = self.BattleDungeonResult(
            battley_key,
            dungeon_id,
            stage_id,
            unit_id_list,
    ...
    def doArena(self, opp_wizard_id, units=None, win_lose=1, log_id=0, is_npc=False):
        if self.pvp_info.get("rating_remained") <= 601200:
            if (
                opp_wizard_id not in [wizard for wizard in self.arena_list]
            ...
            arena_start = (
                self.BattleArenaStart(opp_wizard_id, unit_id_list, log_id)
                if log_id
                else self.BattleArenaStart(opp_wizard_id, unit_id_list)
            )
            if not arena_start:
    ...
    def do_scenario(
        ...
        if any(
            [
                ...
                and region["difficulty"] == difficulty
                and region["stage_list"][-1]["stage_no"] + 1 < stage_no
                for region in self.scenario_list
            ]
        ):
        ...
        if self.wizard_info["wizard_energy"] < int(
            scenarios[find(scenarios, "scenario id", scenario_id)]["energy cost"]
        ):
        ...
        self.log(
            "Battle started: region {}, stage {}, difficulty {}".format(
                region_id, stage_no, difficulty
            )
        )
        unit_result_list = [
            {"unit_id": unit["unit_id"], "pos_id": i + 1}
        ...
        battle_end = self.BattleScenarioResult(
            res[0],
            res[1],
            unit_result_list,
            {
    ...
    def _check_response_data(self, res):
        ...
        if isIn("scenario_info", res):
            try:
                ...
                region_list = [
                    {"i": i, "region": region}
                    for i, region in enumerate(self.scenario_list)
                    if region["region_id"] == input_["region_id"]
                    and region["difficulty"] == input_["difficulty"]
                ]
                if region_list:
                    for region in region_list:
                        try:
                            self.scenario_list[region["i"]]["stage_list"][
                                input_["stage_no"] - 1
                            ]["cleared"] = input_["cleared"]
                        except IndexError:
                            self.scenario_list[region["i"]]["stage_list"].append(
                                {
                                    "stage_no": input_["stage_no"],
                                    "cleared": input_["cleared"],
                                }
                            )
                    ...
                    self.scenario_list.append(
                        {
                            "region_id": input_["region_id"],
                            "difficulty": input_["difficulty"],
                            "cleared": 0,
        ...
        if isIn("harvest_mana", res):
            try:
                self.log(
                    "Harvested {} mana from building {}".format(
                        json.loads(res)["harvest_mana"],
        ...
        if isIn("harvest_crystal", res):
            try:
                self.log(
                    "Harvested {} crystal from building {}".format(
                        json.loads(res)["harvest_crystal"],
                        json.loads(res)["building_info"]["building_id"],
                    )
                )
            except KeyError:
    ...
    def _check_ret_code(self, res, rj):
        if isIn("ret_code", res):
            ...
            if self.debug == 1:
                self.log(
                    "ret_code: {} command: {}".format(rj["ret_code"], rj["command"])
                )
        return rj
